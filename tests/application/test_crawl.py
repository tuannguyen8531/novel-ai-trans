...
def test_run_crawl_dry_run_returns_preview_without_crawling(tmp_path: Path) -> None:
    ...
    with patch("src.application.crawl.crawler.NovelCrawler", return_value=crawler):
        result = run_crawl(
            CrawlRequest(
                novel="example",
...
def test_headed_crawl_forces_one_worker_for_shared_browser_page(tmp_path: Path) -> None:
    ...
    with (
        patch("src.application.crawl.crawler.NovelCrawler", return_value=crawler),
...
def test_import_workflow_reports_chapter_changes_in_result_and_logs() -> None:
    ...
    with patch("src.application.crawl.importer.import_epub", return_value=imported):
