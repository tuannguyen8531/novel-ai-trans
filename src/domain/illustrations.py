...
ILLUSTRATION_MARKER_RE = re.compile(r"\[\[ILLUSTRATION:([A-Za-z0-9][A-Za-z0-9._-]*)\]\]")
...
def restore_illustration_markers(
    ...
    paragraphs = [part.strip() for part in re.split(r"\n\s*\n", translated_text) if part.strip()]
    for inserted, (paragraph_index, marker) in enumerate(placements):
