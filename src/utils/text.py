"""Text normalization and HTML-to-text helpers used by both crawler and translator."""
...
def normalize_text(value: str | None) -> str:
    """Collapse whitespace and strip the result. Replaces NBSP with space."""
    if value is None:
        return ""
...
def html_to_plain_text(container: Tag) -> str:
    """Extract plain text from a BeautifulSoup Tag, preserving paragraph breaks."""
...
def normalize_paragraph_spacing(text: str) -> str:
    """Normalize paragraph spacing to ensure paragraphs are separated by exactly one blank line.
