...
- Translate every source sentence and meaningful beat faithfully and completely.
- Preserve meaning, event order, paragraph structure, and significant formatting.
...
- If numbered, preserve the source number; use "Chương N: <tiêu đề chương đã dịch>".
...
PRIORITY ORDER:
1. Fidelity, completeness, and explicit source-supported changes.
2. Established glossary and character names; confirmed address rules when the source is ambiguous.
3. The target-language prose rules supplied below.
4. Other style preferences only when they do not conflict with the above.
