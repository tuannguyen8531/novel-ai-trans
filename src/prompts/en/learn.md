...
- Judge each verdict from source events, the continuing relationship, relative status, conversational register, and explicit forms of address — never from the translated address forms
...
- Source languages may not lexically distinguish every target-language address choice. Exact source equivalents are not required when the source independently preserves the social relationship and register that justify the candidate
